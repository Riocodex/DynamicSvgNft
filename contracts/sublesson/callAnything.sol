//SPDX-License-Identifier: MIT
 //lesson on how to call functions at a lowlevel language or binary interface

 //in order to call a function using only the data field of call , we need to encode
 //the function and its parameter down to a binary level
...
//this function below is a basic function which we will be able to call w data 
    function transfer(address someAddress , uint256 amount) public {
        s_someAddress = someAddress;
        s_amount = amount;
    }
    //first encode the name of the function
    function getSelectorOne() public pure returns(bytes4 selector){
        selector = bytes4(keccak256(bytes("transfer(address,uint256)")));
    }
    //then the parameters with the encoded function name
    ...
    //encoding with signature..here youll notice we dont need to encode the name of the function
    //the major diff between signature and selector is we dont need to encode the function name
